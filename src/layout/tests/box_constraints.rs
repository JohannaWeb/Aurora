...
fn applies_margin_and_padding_to_block_layout() {
    ...
    assert!(rendered.contains(
        "block<section> {margin: 10px 12px, padding: 4px 6px} [x: 12, y: 10, w: 176, h: 36]"
    ));
    // x = 12 (margin) + 6 (padding) = 18. y = 10 (margin) + 4 (padding) = 14.
    assert!(rendered.contains("text(\"Box\") [x: 18, y: 14, w: 28, h: 22]"));
}
...
fn includes_border_width_in_box_geometry() {
    ...
    assert!(rendered.contains("block<section> {border: 4px solid ember, padding: 6px, width: 80px} [x: 0, y: 0, w: 100, h: 48]"));
    assert!(rendered.contains("text(\"Border\") [x: 10, y: 10, w: 52, h: 22]"));
}
...
fn applies_fixed_width_and_height_to_block_boxes() {
    ...
    assert!(rendered.contains("text(\"Sized\") [x: 4, y: 4, w: 39, h: 22]"));
}
...
fn constrains_inline_wrapping_with_fixed_width() {
    ...
    assert!(rendered.contains("text(\"one two\") [x: 4, y: 4, w: 61, h: 19]"));
